...
function validate () {
    ...
    if(inputCode.length <=0) {
      document.getElementById("J_codetext").setAttribute("placeholder","输入为空");
      createCode();
      return false;
    }
    else if(inputCode != codeToUp ){
      document.getElementById("J_codetext").value="";
      document.getElementById("J_codetext").setAttribute("placeholder","验证码111错误");
      createCode();
      return false;
    }
    else {
		//document.getElementById("J_codetext").setAttribute("placeholder","验证码错误");
      //window.open(document.getElementById("J_down").getAttribute("data-link"));//自己给跳转
      document.getElementById("J_codetext").value="";
      createCode();
      return true;
    }
